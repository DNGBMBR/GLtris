...
public class TopFrame extends Frame implements MouseClickCallback, MouseMoveCallback, MouseScrollCallback {
	public TopFrame(double windowWidth, double windowHeight, boolean isActive) {
		...
		MouseListener.registerMouseScrollCallback(this);
	}
	...
	public void destroy() {
		...
		MouseListener.unregisterMouseScrollCallback(this);
	}
	...
	public void onScroll(long window, double xOffset, double yOffset) {
		...
		mouseX[0] = mouseX[0] / windowWidth[0] * width;
		mouseY[0] = (windowHeight[0] - mouseY[0]) / windowHeight[0] * height;
		this.onScroll(mouseX[0], mouseY[0], xOffset, yOffset);
	}
}
